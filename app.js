...
/* ============================ Dashboard ============================ */
function Dashboard({ config, counts, totalKm, onCapture, onImport, onOptimize, onConfig, optimizing, hasRoute, userName, isOwner, onTeam, rotaNum, dayTotals, hasClosedRoutes, onNovaRota }) {
...
function ConferSheet({ dados, photo, onConfirm, onCancel }) {
  ...
  const add = (keepGoing) => {
    ...
    onConfirm({ ...d, endereco: enderecoFinal }, keepGoing);
  };
...
/* ============================ Stop detail sheet ============================ */
function StopSheet({ stop, onClose, onUpdate, onDelete, onEditAddress, onRemovePkg }) {
  const g = stop.geo || {};
  const [editing, setEditing] = useState(false);
  const [f, setF] = useState({
    nome: stop.name || "", endereco: g.endereco || stop.address || "", bairro: g.bairro || "",
  ...
  if (editing) {
    return html`<${Sheet} title="Editar endereço" sub="Corrija o que a IA leu errado" onClose=${() => setEditing(false)}>
      ...
      <div class="field"><label>Endereço (rua, nº, compl.)</label><input class="input" value=${f.endereco} onInput=${set("endereco")} placeholder="Ex.: Av. Paulino Braga, 426" /></div>
      <div class="grid2">
        <div class="field"><label>Bairro</label><input class="input" value=${f.bairro} onInput=${set("bairro")} /></div>
        <div class="field"><label>CEP</label><input class="input" value=${f.cep} onInput=${set("cep")} /></div>
      </div>
      <div class="grid2">
        <div class="field"><label>Cidade</label><input class="input" value=${f.cidade} onInput=${set("cidade")} /></div>
        <div class="field"><label>UF</label><input class="input" value=${f.uf} onInput=${set("uf")} /></div>
      </div>
      <div class="field"><label>Código</label><input class="input" value=${f.codigo} onInput=${set("codigo")} /></div>
      <button class="btn btn--lg" onClick=${() => { if (!f.endereco.trim()) return toast("Preencha o endereço", "err"); onEditAddress(stop.id, f); setEditing(false); onClose(); }}>
...
function App() {
  ...
  const [state, setState] = useState(() => {
    ...
    if (s.dia !== hoje) return { ...s, dia: hoje, stops: [], lastKm: 0, atualizadoEm: 0, rotaNum: 1, rotasFechadas: [] };
    return s;
  });
    ...
    (async () => {
      ...
      try {
        ...
        if (remote && Array.isArray(remote.stops) && remoteTs > localTs) {
          ...
          setState((s) => ({ ...s, stops: remote.stops, lastKm: remote.lastKm || 0, dia: hoje, atualizadoEm: remoteTs, rotaNum: remote.rotaNum || 1, rotasFechadas: remote.rotasFechadas || [] }));
        } else {
          ...
          cloud.saveDay(cloudUserId, hoje, {
            ...
            rotaNum: local.rotaNum || 1, rotasFechadas: local.rotasFechadas || [],
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      cloud.saveDay(cloudUserId, hoje, {
        ...
        rotaNum: stateRef.current.rotaNum || 1, rotasFechadas: stateRef.current.rotasFechadas || [],
        entregador: session?.nome || cloudUserId, atualizadoEm: ts,
      });
      setState((s) => ({ ...s, atualizadoEm: ts }));
    }, 800);
    return () => clearTimeout(t);
  }, [stops, state.lastKm, state.rotaNum]);
  ...
  const addFromConf = (d) => {
    ...
    addStops([{
      ...
      geo: { endereco: (d.endereco || "").trim(), bairro: (d.bairro || "").trim(), cidade: (d.cidade || "").trim(), uf: (d.uf || "").trim(), cep: (d.cep || "").trim() },
  ...
  const novaRota = () => {
    ...
    const aviso = falta > 0
      ? `Ainda há ${falta} pacote(s) não entregue(s) nesta rota. Fechar mesmo assim e começar a Rota ${rotaNum + 1}?`
      : `Fechar a Rota ${rotaNum} e começar a Rota ${rotaNum + 1}?`;
    if (!confirm(aviso)) return;
    const resumo = {
      id: uid(), num: rotaNum,
      pacotes: counts.total, entregues: counts.delivered, falhas: counts.failed,
      km: state.lastKm || 0, fechadaEm: Date.now(),
    };
    setState((s) => ({
      ...s,
      rotasFechadas: [...(s.rotasFechadas || []), resumo],
      stops: [], lastKm: 0, rotaNum: (s.rotaNum || 1) + 1,
    }));
    setView("home");
    toast(`Rota ${rotaNum} fechada • começando Rota ${rotaNum + 1}`, "ok");
  };
  ...
  const editStopAddress = (id, d) => {
    const address = montarEndereco(d);
    const key = addrKey(d);
    const geo = { endereco: (d.endereco || "").trim(), bairro: (d.bairro || "").trim(), cidade: (d.cidade || "").trim(), uf: (d.uf || "").trim(), cep: (d.cep || "").trim() };
    let merged = false;
    setStops((cur) => {
