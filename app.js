...
import { login, getSession, isLogged, logout } from "./modules/auth.js";
...
function LoginScreen({ onLogged }) {
  ...
  const entrar = async () => {
    if (!nome.trim()) return toast("Digite seu nome", "err");
    setBusy(true);
    const ok = await login(senha, nome);
    setBusy(false);
    if (ok) onLogged();
  ...
  return html`<div class="login">
    ...
    <div class="login__card">
      <div class="card__title" style="margin-bottom:4px">Acesso do entregador</div>
      <p class="muted small" style="margin-top:0">Entre para começar o dia.</p>
      <div class="field"><label>Seu nome</label>
        <input class="input" value=${nome} onInput=${(e) => setNome(e.target.value)} placeholder="Ex.: André" /></div>
    ...
    return html`<div class="cap">
      ...
      <div class="pick">
        ...
        ${list.map((s) => html`<button class="pick__item" onClick=${() => choose(s)}>
          ...
          <div class="pick__info"><div class="pick__name">${s.name}</div><div class="pick__addr">${s.address}</div></div>
        </button>`)}
...
function NumberResult({ result, onNext, onClose }) {
  ...
  let cls = "scanres--ok", label = "", num = "", name = "", addr = "", msg = "";
  if (result.kind === "ok-separate") { label = "Parada nº"; num = result.stop.order; name = result.stop.name; addr = result.stop.address; }
    ...
    <div class="map-wrap" style="position:relative;flex:1">
      ...
      ${hasRoute ? html`<div class="route-bar">
        ...
        <button class=${"route-bar__nav " + (nav ? "is-on" : "")} onClick=${() => setNav((v) => !v)}>
          <${Icon} name="location" width="18" height="18"/> ${nav ? "Parar" : "Iniciar navegação"}
        </button>
      </div>` : null}
    </div>
  </div>`;
...
function App() {
  ...
  // vindo da conferência (1 etiqueta lida pela IA)
  const addFromConf = (d) => {
    const address = montarEndereco(d);
    addStops([{
      id: uid(), code: (d.codigo || "").trim(), name: (d.nome || "").trim() || "Sem nome",
      address, phone: (d.telefone || "").trim(), note: "", lat: undefined, lng: undefined,
