...
/* ============================ Dashboard ============================ */
function Dashboard({ config, counts, totalKm, onCapture, onImport, onOptimize, onConfig, optimizing, hasRoute, userName, isOwner, onTeam, rotaNum, dayTotals, hasClosedRoutes, onNovaRota, onRotasDia }) {
...
function RotasDoDiaSheet({ userId, userName, rotaAtual, onClose, onAbrirRota, onCriarRota }) {
  ...
  const statusLabel = { ativa: "Em andamento", preparando: "Preparando", fechada: "Fechada" };
  const statusClass = { ativa: "chip--separated", preparando: "chip--warn", fechada: "chip--delivered" };
  ...
  return html`<${Sheet} title="Rotas de hoje" sub=${userName} onClose=${onClose}>
    ...
    ${rotas === null
      ...
      : rotas.length === 0
        ? html`<p class="muted small">Nenhuma rota na nuvem ainda hoje.</p>`
        : rotas.map((r) => {
            ...
            return html`<div class="rotaline">
              <div class="rotaline__info">
                <div class="rotaline__title">Rota ${r.num} ${isAtual ? html`<span class="muted small">(neste celular)</span>` : ""}</div>
                <div class="muted small">${pacotes} pacote${pacotes !== 1 ? "s" : ""} • ${entregues} entregue${entregues !== 1 ? "s" : ""}</div>
              </div>
              <span class=${"chip " + (statusClass[r.status] || "chip--warn")}>${statusLabel[r.status] || r.status || "—"}</span>
              ${!isAtual ? html`<button class="btn btn--ghost rotaline__btn" onClick=${() => onAbrirRota(r.num)}>Abrir aqui</button>` : null}
            </div>`;
          })}
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      cloud.saveDay(cloudUserId, hoje, {
        ...
        rotaNum: rNum, rotasFechadas: stateRef.current.rotasFechadas || [],
