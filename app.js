...
import { initCloud, cloudReady, getDay, saveDay, getTeamDay } from "./modules/cloud.js";
...
/* ============================ Dashboard ============================ */
function Dashboard({ config, counts, totalKm, onCapture, onImport, onOptimize, onConfig, optimizing, hasRoute, userName, isOwner, onTeam }) {
...
function EquipeSheet({ onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const hoje = todayISO();
      const data = await getTeamDay(hoje, [["andre", "André"], ["guilherme", "Guilherme"]]);
  ...
  const countOf = (stops = []) => {
    ...
    return { total: sum(() => true), delivered: sum((s) => s.status === "delivered"), failed: sum((s) => s.status === "failed") };
  };
  return html`<${Sheet} title="Equipe — hoje" sub=${today()} onClose=${onClose}>
    ${rows === null
      ...
      : rows.map(({ nome, data }) => {
          ...
          const c = countOf(stops);
          const km = (data && data.lastKm) || 0;
          ...
          return html`<div class="team">
            ...
            <div class="team__stats">
              ...
              <div class="team__stat"><b>${fmtKm(km)}</b><span>km</span></div>
            </div>
          </div>`;
        })}
...
function App() {
  ...
  const [state, setState] = useState(() => {
    ...
    if (s.dia !== hoje) return { ...s, dia: hoje, stops: [], lastKm: 0, atualizadoEm: 0 };
    return s;
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const hoje = todayISO();
        const remote = await getDay(cloudUserId, hoje);
        if (cancel) return;
        ...
        if (remote && Array.isArray(remote.stops) && remoteTs > localTs) {
          ...
          setState((s) => ({ ...s, stops: remote.stops, lastKm: remote.lastKm || 0, dia: hoje, atualizadoEm: remoteTs }));
        } else {
          // local é a fonte -> garante que a nuvem tenha o estado atual
          saveDay(cloudUserId, hoje, {
