...
// ponto do grupo mais próximo de quem está chegando — a "porta de entrada".
// Com a base a 25 km, a diferença entre ruas vista de longe é ruído; ancorar aqui
// faz a rota ENTRAR pela borda de chegada e varrer o bairro a partir dela.
function nearestPointTo(p, stopsList) {
  let best = stopsList[0], bd = Infinity;
  for (const s of stopsList) {
...
export async function optimizeRoute(origin, stops, opts = {}) {
  ...
  for (const cidade of cidadesOrdenadas) {
    ...
    for (const cl of clustersOrdenados) {
      // 5) dentro do grupo: rua por rua, começando pela PORTA (borda de chegada)
      const r = routeHood(nearestPointTo(cur, cl.stops), cl.stops);
