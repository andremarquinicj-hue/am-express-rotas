...
// Limpa o cache de endereços (incl. as buscas que falharam e ficaram salvas como "não achei").
export function limparGeocache() {
...
// Faz UMA busca via /api/geocode (servidor). Retorna {lat,lng} ou null (e cacheia, inclusive a ausência).
async function geocodeQuery(q, token, proximity) {
  ...
  if (token) params.set("token", token); // fallback caso MAPBOX_TOKEN não esteja setado no servidor
  ...
  if (res.status === 401 || res.status === 403) throw new Error("Mapbox bloqueou o acesso (" + res.status + "). Verifique a chave/restrição no Mapbox.");
  if (res.status === 429) throw new Error("Mapbox: limite de uso atingido (429). Tente mais tarde.");
  if (!res.ok) throw new Error("geocode http " + res.status);
  const data = await res.json();
  let out = null;
  if (data && data.lat != null && data.lng != null) out = { lat: data.lat, lng: data.lng };
