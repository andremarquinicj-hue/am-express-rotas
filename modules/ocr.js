...
// Espera d.endereco já com o número (rua + número são juntados antes de chegar aqui).
export function montarEndereco(d) {
  const partes = [d.endereco, d.complemento, d.bairro, d.cidade, d.uf].map((x) => (x || "").trim()).filter(Boolean);
  let s = partes.join(", ");
