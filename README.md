...
1. **Login** → cada pessoa entra com a sua senha (a senha identifica quem é).
...
1. **Entre** com a sua senha. André (dono): `andre2026` · Guilherme (entregador): `guilherme2026`. Troque depois, veja abaixo.
...
### 🔒 Usuários e senhas
Dois usuários em `modules/auth.js`: **André** (dono, senha `andre2026`) e **Guilherme** (entregador, senha `guilherme2026`). A senha digitada identifica quem é. Para trocar uma senha, gere o SHA-256 da nova e substitua o `hash` do usuário no array `USERS`. (Na Fase 2, vira login individual + relatório da equipe na nuvem via Firebase — aí o dono vê os números de cada entregador.)
...
A busca de endereços (geocoding) passa pelo **servidor** (`/api/geocode`), então **não sofre com restrição de domínio** (erro 403) no PWA. Para isso, configure no Vercel a variável de ambiente **`MAPBOX_TOKEN`** com o seu token público do Mapbox (o mesmo `pk.eyJ...`). O token do mapa (tiles) continua no app (`modules/storage.js`). Recomendado deixar o token **sem restrição de URL**, já que do lado do servidor não há domínio de origem.
